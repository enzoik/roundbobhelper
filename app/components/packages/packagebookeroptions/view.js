define(function(require, exports, module) {
  ...
  module.exports = Backbone.Layout.extend({
	...
	afterRender : function() {
		...
		var did = current_url.split("#surprise")[1].split("/")[2].split("_");
		console.log("length",did.length);
		if(did.length < 6){
			$("#view-packages-list").show();
		}else{
			
			$("#view-packages-list").hide();
		}

	},
	events: {
      'click .email_quotationpackages' : 'email_quotation',
      'click .call_clientpackages' : 'call_client',
      'click .available_packagess' : 'available_flights',
    },
	email_quotation:function(){
		...
		var last_check = "";
		if(current_url.split("#surprise")[1].split("/").length > 2){
			var last_spliter = current_url.split("#surprise")[1].split("/")[2];
...
//Spain_(MWX) Mua/2017-11-14__oneway_s/s/single/email/sdng_mail/p234/sent_to
	...
	call_client:function(){
		...
		var splited = current_url.split("#surprise")[1].split("/");
		var redirectTo = '/surprise';
		if(url_.split('/')[3] == "m"){
			console.log("many travellers");
...
//http://localhost/roundbobhelperv1/dist/#flights/(EBB) Entebbe, Entebbe Uganda_(NBO) Jomo Kenyatta Intl., Nairobi Kenya_Economy/2017-11-07_2017-11-13_round_s/2017-11-07_2017-11-13_round_s/2017-11-07_2017-11-13_round_s/s/single/true_/calling/sen_to/s/single/true_/calling/sen_to
	...
	available_flights:function(){
		console.log("Search for surprise");
		var current_url  = window.location.href.toString();
		//http://localhost/roundbobhelperv1/dist/#surprise/4_City%20getaway_1_Uganda/2017-12-19_2017-12-28_round_m/m/adlts_1_chldn_1_infnts_0
		//http://localhost/roundbobhelperv1/dist/#surprise/4_City getaway_1_Uganda/2017-12-13_2017-12-28_s
		// http://localhost/roundbobhelperv1/dist/#surprise/4_City getaway_1_Uganda/2017-12-13_2017-12-31_m/m/adlts_1_chldn_1_infnts_0
		//http://localhost/roundbobhelperv1/dist/#surprise/4_City getaway_1_Uganda/2017-12-13_2017-12-31_m/m/adlts_1_chldn_1_infnts_0/email/m_mail/m_client/m_summary/patrick_patrickkanyerezi@gmail.com_email
		//http://localhost/roundbobhelperv1/dist/#surprise/4_City getaway_1_Uganda/m/adlts_1_chldn_0_infnts_0/email/m_mail/m_client/m_summary/mydetails/results
		//http://localhost/roundbobhelperv1/dist/#surprise/4_City getaway_1_Uganda/m/adlts_1_chldn_0_infnts_0/email/m_mail/m_client/m_summary/mydetails/results/packages
		
		var splitted = current_url.split("#surprise")[1];
		var no_ = current_url.split("#surprise")[1].split("/")[2].split("_")[3];
		console.log("no_"+current_url.split("#surprise")[1].split("/").length );
		var redirectTo="";
		 if(current_url.split("#surprise")[1].split("/").length > 8){
