...
//Reservations123!@#
    onPackageItemSelected: function(ev) {
		console.log("on selected a package" +ev);
		
    var id = this.model.cid;
console.log("details feed",this.model.attributes.Destination.id);     
	 var resultIndex = this.model.attributes.Destination.id;

      // Easily create a URL.
      app.router.go("packages","popular","details", id, resultIndex);
