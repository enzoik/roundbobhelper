  ...
  module.exports = Backbone.Layout.extend({
	...
	/*	}else if(!roundtrip.checked ){
		...
		//	console.log("multiple "+departure_date+" return "+return_date+"trip type"+travel_trip+"current_url"+current_url);
			...
			console.log("length",current_url.split("#surprise").length);
		...
		var redirectTo = '';
		if(last_check =="m" && current_url.split("#surprise")[1].split("/").length > 2 ){
			
		}else if(current_url.split("#surprise").length < 3)
			{	
				redirectTo = '/surprise';
				redirectTo += current_url.split("#surprise")[1];
			   ...
			   app.router.go(redirectTo);
				...
				redirectTo = '/surprise';
				redirectTo += current_url.split("#surprise")[1].split("/")[0];
			  redirectTo += '/' + departure_date+"_"+return_date+"_"+travel_trip+"_m/m";
				console.log("multiple_view",redirectTo);
			  app.router.go(redirectTo);
			}
	...
	alonepeopleGetDate:function(e){
		...
		var departure_date = document.getElementById("departure-date").value;
		var return_date = document.getElementById("return-date").value;
		var roundtrip = document.getElementById("switch-123");
		var travel_trip="";
		var redirectTo="";
		if(!roundtrip.checked){
			travel_trip="oneway";
